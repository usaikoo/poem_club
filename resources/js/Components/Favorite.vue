...
//fun for add or remove favorite
const toggleFavorite = () => {
    router.visit(`/blog/${props.id}/toggle-favorite`, {
        method: 'post',
    }) 
}
const page = usePage()
...
onMounted(() => {
    isFavorite.value = page.props.auth.favorites.includes(props.id)
})
</script>
...
<template>
    <button @click="toggleFavorite"
        ...
        <svg v-if="!isFavorite" viewBox="0 0 24 24" class="w-6" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg v-else viewBox="0 0 24 24" class="w-6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
            <g id="SVGRepo_iconCarrier">
                <path
